...
SNAPSHOT_FILE = "scanner_previous_snapshot.csv"
...
def classify_bias(row):
    ...
    pcr = row.get("nearby_pcr", np.nan)

    if pd.isna(spot) or pd.isna(call_wall) or pd.isna(put_wall):
        return "Long Volatility"

    if not pd.isna(rvol) and rvol >= MIN_RVOL:
        if spot > put_wall and (pd.isna(pcr) or pcr >= 0.8):
            return "Long Call"

        if spot < call_wall and (not pd.isna(pcr) and pcr <= 0.8):
            return "Long Put"
...
def build_chain_features(details):
    rows = []

    for symbol, g in details.groupby("symbol"):
        g = g.copy()

        spot = safe_float(g["spot"].dropna().iloc[0]) if g["spot"].notna().any() else np.nan

        total_call_oi = g["ce_oi"].sum()
        total_put_oi = g["pe_oi"].sum()
        total_oi = total_call_oi + total_put_oi

        pcr = total_put_oi / total_call_oi if total_call_oi > 0 else np.nan

        if not pd.isna(spot):
            g["distance_pct"] = ((g["strike"] - spot).abs() / spot) * 100
            nearby = g[g["distance_pct"] <= 3].copy()
        else:
            nearby = g.copy()

        nearby_call_oi = nearby["ce_oi"].sum()
        nearby_put_oi = nearby["pe_oi"].sum()
        nearby_pcr = nearby_put_oi / nearby_call_oi if nearby_call_oi > 0 else np.nan

        avg_ce_iv = g["ce_iv"].replace(0, np.nan).mean()
        avg_pe_iv = g["pe_iv"].replace(0, np.nan).mean()
        avg_chain_iv = np.nanmean([avg_ce_iv, avg_pe_iv])

        call_wall_row = g.sort_values("ce_oi", ascending=False).head(1)
        put_wall_row = g.sort_values("pe_oi", ascending=False).head(1)

        call_wall_strike_all = call_wall_row["strike"].iloc[0] if not call_wall_row.empty else np.nan
        put_wall_strike_all = put_wall_row["strike"].iloc[0] if not put_wall_row.empty else np.nan

        atm_row = g.iloc[(g["strike"] - spot).abs().argsort()[:1]] if not pd.isna(spot) else g.head(1)

        atm_ce_oi = atm_row["ce_oi"].iloc[0] if not atm_row.empty else np.nan
        atm_pe_oi = atm_row["pe_oi"].iloc[0] if not atm_row.empty else np.nan
        atm_pcr = atm_pe_oi / atm_ce_oi if atm_ce_oi and atm_ce_oi > 0 else np.nan
        ...
        rows.append(
            {
                ...
                "call_wall_strike_all": call_wall_strike_all,
                "put_wall_strike_all": put_wall_strike_all,
            }
        )
...
def load_previous_snapshot():
    ...
    try:
        ...
        prev = normalize_input_columns(prev)
        if "symbol" in prev.columns:
            prev["symbol"] = prev["symbol"].apply(clean_symbol)
        return prev
    except Exception:
        return pd.DataFrame()


def add_snapshot_deltas(df):
    ...
    keep_cols = [c for c in keep_cols if c in prev.columns]
    prev = prev[keep_cols].copy()
    prev = prev.rename(columns={c: f"prev_{c}" for c in keep_cols if c != "symbol"})

    df = pd.merge(df, prev, on="symbol", how="left")

    df["rank_score_change"] = df["rank_score"] - df.get("prev_rank_score", np.nan)
    df["trade_score_change"] = df["trade_score"] - df.get("prev_trade_score", np.nan)
    df["pcr_change"] = df["pcr"] - df.get("prev_pcr", np.nan)
    df["nearby_pcr_change"] = df["nearby_pcr"] - df.get("prev_nearby_pcr", np.nan)
    df["atm_iv_change"] = df["atm_iv"] - df.get("prev_atm_iv", np.nan)
    df["total_oi_change"] = df["total_oi"] - df.get("prev_total_oi", np.nan)
    df["call_wall_shift"] = df["max_call_oi_strike"] - df.get("prev_max_call_oi_strike", np.nan)
    df["put_wall_shift"] = df["max_put_oi_strike"] - df.get("prev_max_put_oi_strike", np.nan)
...
def save_snapshot(df):
    ...
    snapshot_cols = [c for c in snapshot_cols if c in snap.columns]
    snap[snapshot_cols].to_csv(SNAPSHOT_FILE, index=False)
...
def assign_candidate_quality(row):
    ...
    if (
        ...
        and contract_oi >= MIN_CONTRACT_OI
    ):
        return "A - Clean"

    if (
        breakeven <= MAX_BREAKEVEN_PCT
        and max_loss <= MAX_LOSS_LIMIT
        and rvol >= MIN_RVOL
    ):
        return "B - Tradable"
...
def assign_confidence_grade(row):
    ...
    if quality == "A - Clean" and score >= 45:
        return "A"
...
def assign_liquidity_flag(row):
    oi = row.get("contract_oi", np.nan)
    premium = row.get("estimated_premium", np.nan)

    if pd.isna(oi) or oi <= 0:
        return "LOW_OI"

    if oi < MIN_CONTRACT_OI:
        return "THIN"
...
def build_reason(row):
    ...
    max_loss = row.get("estimated_max_loss", np.nan)
    pcr = row.get("nearby_pcr", np.nan)
    liquidity = row.get("liquidity_flag", "")

    if not pd.isna(hv20) and not pd.isna(hv60):
        reasons.append("HV expanding" if hv20 > hv60 else "HV not expanding")

    if not pd.isna(atm_iv) and not pd.isna(hv20):
        if atm_iv <= hv20:
            reasons.append("IV cheap vs HV")
        elif atm_iv <= hv20 * 1.25:
            reasons.append("IV reasonable")
        else:
            reasons.append("IV expensive")

    if not pd.isna(rvol):
        reasons.append("Strong RVOL" if rvol >= MIN_RVOL else "Weak RVOL")

    if not pd.isna(breakeven):
        if breakeven <= 5:
            reasons.append("Acceptable option cost")
        elif breakeven <= MAX_BREAKEVEN_PCT:
            reasons.append("Slightly expensive option")
        else:
            reasons.append("Expensive option")

    if not pd.isna(max_loss):
        reasons.append("Capital fit" if max_loss <= MAX_LOSS_LIMIT else "Capital too high")

    if not pd.isna(pcr):
        if pcr >= 1.2:
            reasons.append("Put support strong")
        elif pcr <= 0.7:
            reasons.append("Call pressure strong")
        else:
            reasons.append("PCR balanced")
...
def main():
    ...
    for name, frame in [
        (VOLATILITY_FILE, vol),
        (OPTION_CHAIN_FILE, opt),
        (OPTION_CHAIN_DETAILS_FILE, details),
    ]:
        if "symbol" not in frame.columns:
            raise Exception(f"{name} missing required column: symbol")
    ...
    for col in ["strike", "spot", "ce_ltp", "pe_ltp", "ce_iv", "pe_iv", "ce_oi", "pe_oi"]:
        ensure_col(details, col)
        details[col] = details[col].apply(safe_float)

    chain_features = build_chain_features(details)

    df = pd.merge(vol, opt, on="symbol", how="inner")
    df = pd.merge(df, chain_features, on="symbol", how="left")
    ...
    ] = df.apply(lambda row: suggest_tradable_option(row, details), axis=1)
    ...
    df["liquidity_flag"] = df.apply(assign_liquidity_flag, axis=1)
    ...
    df.loc[df["liquidity_flag"] == "OK", "rank_score"] += 3

    df["rank_score"] = df["rank_score"].round(2)

    df = add_snapshot_deltas(df)
