...
def rename_if_exists(df, variants, final_name):
    for v in variants:
        v = normalize_col_name(v)
        if v in df.columns:
            df.rename(columns={v: final_name}, inplace=True)
            return df
    return df
...
def normalize_input_columns(df):
    ...
    rename_if_exists(df, ["trading_symbol", "symbol_name", "sem_trading_symbol"], "symbol")
    rename_if_exists(df, ["hv20", "hv_20", "historical_volatility_20", "historical_volatility20"], "hv20")
    rename_if_exists(df, ["hv60", "hv_60", "historical_volatility_60", "historical_volatility60"], "hv60")
    rename_if_exists(df, ["relative_volume", "rel_volume", "rvol", "rv"], "relative_volume")
...
def main():
    ...
    vol = normalize_input_columns(vol)
    opt = normalize_input_columns(opt)
    details = normalize_input_columns(details)

    required_vol_cols = ["symbol"]
    for col in required_vol_cols:
        if col not in vol.columns:
            raise Exception(f"{VOLATILITY_FILE} missing required column: {col}")
